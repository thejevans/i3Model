...
for i, event in enumerate(all_events):
    ...
    max_charge = max(thisevent.T[3])
    brightness = (thisevent.T[3]*max_brightness/max_charge).astype(int)
    ...
    strings = thisevent.T[2]%2+1 ## remainder 0 = 1st string; remainder 1 = 2nd string
    #strings = thisevent.T[2]
    for j, pulse in enumerate(thisevent):
        print ('### Pulse {0}: {1}, {2}, {3}, {4}'.format(j, wavelength[j], thisevent[j][1], thisevent[j][2], brightness[j]))
